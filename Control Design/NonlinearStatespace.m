...
% Calculate control action (in terms of rotor speed)
u = ue'-K*(x-xe');
...
% Calculate first 6 states (x,y,z,roll,pitch,yaw)
xdot(1) = x(7);
xdot(2) = x(8);
xdot(3) = x(9);
xdot(4) = x(10);
...
% Calculate roll,pitch,yaw rates
ndot = [x(10) x(11) x(12)]';
ndotdot = inv(J) * (tb - (C*ndot));
xdot(10) = ndotdot(1);
xdot(11) = ndotdot(2);
xdot(12) = ndotdot(3);
xdot=xdot';
